export default {
  async fetch(request, env, ctx) {
    ...
    const isValid = (w) => {
      ...
      if (l.startsWith('{') || l.startsWith('[') || l.includes('"id"') || l.includes('"lam"') || l.includes('"senses"') || l.includes('"eng"')) return false;
      if (l.length > 80 || e.length > 300) { if (l.includes('id') && l.includes('lam') && l.includes('senses')) return false; }
    ...
    if (url.pathname === "/api/check-access") {
      ...
      let sub = await DICT.get("sub_" + body.code, "json");
    ...
    if (url.pathname === "/api/subscribe") {
      let body = {}; try { body = await request.json(); } catch(e) {}
      let existing = await DICT.get("sub_" + body.code, "json");
...
<div class="card">
  ...
  <input id="searchBox" placeholder="Search Lamnso or English..." oninput="handleSearch()">
  <div style="display:flex;flex-wrap:wrap;margin-top:8px">
    <button class="btn btn-primary" onclick="addWord()">+ Add Word (LOCAL)</button>
    <button class="btn btn-blue" onclick="loadWorld()">Load WORLD</button>
    <button class="btn btn-blue" style="border:2px solid #fff" onclick="pushToWorld()">Push to WORLD Global</button>
    <button class="btn btn-gray" onclick="exportData()">Export LOCAL</button>
...
<div class="card" style="border:2px dashed var(--primary);background:#e8f5e980">
  <span class="label">IMPORT - SEPARATE FROM PUSH</span>
  <h3 style="margin:4px 0">Paste Multiple Words Here</h3>
  <textarea id="importBox" placeholder="Paste JSON or lines like: wan - child"></textarea><br>
...
<div class="card">
  ...
  <div style="display:flex;flex-wrap:wrap">
    ...
    <button class="btn btn-gray" onclick="mergeDuplicates()">Merge Duplicates</button>
    <button class="btn btn-red" onclick="removeDuplicates()">Remove Duplicates</button>
    <button class="btn btn-primary" onclick="startLearn()">Learn Mode</button>
  </div>
</div>
<div class="card">
  <h3>Subscription - Premium Access</h3>
  ...
  <div>
    ...
    <button class="btn btn-gray" onclick="generateCode()">Generate Code (Admin)</button>
  </div>
...
function initTrial(){ if(!localStorage.getItem(TRIAL_KEY)){ localStorage.setItem(TRIAL_KEY, Date.now().toString()); } checkTrial(); }
function checkTrial(){ let start = parseInt(localStorage.getItem(TRIAL_KEY) || '0'); if(!start){ localStorage.setItem(TRIAL_KEY, Date.now().toString()); start = Date.now(); } let daysPassed = (Date.now()-start)/(1000*60*60*24); let daysLeft = 7 - daysPassed; let codeOk = localStorage.getItem(PREMIUM_KEY); let banner = document.getElementById('trialBanner'); let paywall = document.getElementById('paywall'); if(codeOk){ banner.style.display='block'; banner.style.background='#1565c0'; banner.innerText='Premium Active - Thank you! MoMo: 674 061 571'; paywall.style.display='none'; return; } if(daysLeft > 0){ banner.style.display='block'; banner.style.background='#0b5e2f'; banner.innerText='Free Trial: '+Math.ceil(daysLeft)+' days left | After trial: 10,000 FCFA/year - MoMo: 674 061 571'; paywall.style.display='none'; } else { banner.style.display='none'; paywall.style.display='flex'; } } initTrial();
const isValid = (w) => { if (!w) return false; const l = (w.lamnso || w.lam || w.word || '').toString().trim(); const e = (w.english || w.eng || w.meaning || w.definition || (w.senses && w.senses[0] && w.senses[0].eng) || '').toString().trim(); if (!l ||!e) return false; if (l.toLowerCase() === 'undefined' || e.toLowerCase() === 'undefined') return false; if (l.startsWith('{') || l.startsWith('[') || l.includes('"id"') || l.includes('"lam"') || l.includes('"senses"')) return false; if (e.startsWith('{') || e.includes('"senses"')) return false; return true; };
const clean = (arr) => (arr||[]).filter(isValid).map(w=>{ let lam = (w.lamnso || w.lam || w.word || '').toString().trim().replace(/^["']|["']$/g,'').replace(/"/g, "'"); let eng = (w.english || w.eng || w.meaning || w.definition || (w.senses && w.senses[0] && w.senses[0].eng) || '').toString().trim().replace(/^["']|["']$/g,'').replace(/"/g, "'"); let pos = (w.pos || (w.senses && w.senses[0] && w.senses[0].pos) || 'n').toString().trim().toLowerCase(); if (pos.length > 20) pos = 'n'; return {lamnso: lam, english: eng, pos: pos, id: w.id || Date.now()+Math.random()}; });
localData = clean(localData); saveLocal();
async function autoLoadForNewVisitor(){ if(localData.length === 0){ try{ document.getElementById('status').innerText = 'Loading WORLD...'; let r = await fetch(WORLD_URL); if(r.ok){ let data = await r.json(); data = clean(data); if(data.length > 0){ localData = data; saveLocal(); render(); document.getElementById('status').innerText = 'WORLD Auto-Loaded'; } else { document.getElementById('status').innerText = 'LOCAL empty - Tap Load WORLD'; } } }catch(e){ document.getElementById('status').innerText = 'LOCAL empty - Tap Load WORLD'; } } }
function toggleDark(){ document.body.classList.toggle('dark'); localStorage.setItem('dark', document.body.classList.contains('dark')) } if(localStorage.getItem('dark')==='true') document.body.classList.add('dark');
function render(data){ if(!data) data = localData; data = clean(data); document.getElementById('wordCount').innerText = data.length + ' words'; let html = ''; for(let idx=0; idx<Math.min(data.length,500); idx++){ let w = data[idx]; let realIdx = -1; for(let j=0;j<localData.length;j++){ if(localData[j].lamnso===w.lamnso && localData[j].english===w.english){ realIdx=j; break; } } if(realIdx===-1) realIdx = idx; html += '<div class="word-row"><div><b>'+w.lamnso+'</b> - '+w.english+' <small>['+w.pos+']</small></div><div><button class="btn btn-yellow" style="padding:5px 8px" onclick="editWord('+realIdx+')">Edit</button><button class="btn btn-red" style="padding:5px 8px" onclick="deleteWord('+realIdx+')">Del</button></div></div>'; } if(!html) html = '<small>No valid words - Use IMPORT box or Load WORLD</small>'; document.getElementById('list').innerHTML = html; } render(); autoLoadForNewVisitor();
function handleSearch(){ let q = document.getElementById('searchBox').value.toLowerCase(); let f = localData.filter(w=> JSON.stringify(w).toLowerCase().includes(q)); render(f); }
function addWord(){ let lam = prompt('Lamnso word:'); if(!lam || lam.toLowerCase()==='undefined') return; let eng = prompt('English meaning:'); if(!eng || eng.toLowerCase()==='undefined') return; localData.push({lamnso:lam.trim(), english:eng.trim(), pos:'n', id:Date.now()}); saveLocal(); render(); document.getElementById('status').innerText='LOCAL - Not yet pushed'; }
function editWord(i){ let w=localData[i]; if(!w) return; let l=prompt('Edit Lamnso:',w.lamnso); if(l===null) return; let e=prompt('Edit English:',w.english); if(e===null) return; if(!l.trim()||!e.trim()) return; localData[i]={lamnso:l.trim(), english:e.trim(), pos:w.pos, id:w.id}; saveLocal(); render(); }
...
function bulkImport(){ let text=document.getElementById('importBox').value.trim(); if(!text) return alert('Paste words in box first'); let imported=[]; let triedJson = false; if(text.startsWith('[') || text.startsWith('{')){ try { let data = JSON.parse(text); if(!Array.isArray(data)) data = [data]; data.forEach(function(item){ let lam = (item.lamnso || item.lam || item.word || '').toString().trim(); let eng = (item.english || item.eng || item.meaning || item.definition || (item.senses && item.senses[0] && item.senses[0].eng) || '').toString().trim(); let pos = (item.pos || (item.senses && item.senses[0] && item.senses[0].pos) || 'n').toString().trim(); if(lam && eng &&!lam.startsWith('{') &&!lam.includes('"id"')){ imported.push({lamnso: lam.replace(/"/g,"'"), english: eng.replace(/"/g,"'"), pos: pos, id: item.id || Date.now()+Math.random()}); } }); triedJson = true; } catch(e){ triedJson = false; } } if(!triedJson){ let lines=text.split('\\n'); for(let line of lines){ line=line.trim(); if(!line) continue; if(line.startsWith('{') || line.startsWith('[') || line.includes('"id"') || line.includes('"lam"') || line.includes('"senses"')) continue; let parts=null; if(line.includes(' - ')) parts=line.split(' - '); else if(line.includes(' | ')) parts=line.split(' | '); else if(line.includes('|')) parts=line.split('|'); else if(line.includes(' : ')) parts=line.split(' : '); else if(line.includes(':')) parts=line.split(':'); else if(line.includes(',')) parts=line.split(','); else if(line.includes('-')) parts=line.split('-'); else continue; if(parts && parts.length>=2){ let lam=parts[0].trim().replace(/^["']|["']$/g,''); let eng=parts.slice(1).join(' ').trim().replace(/^["']|["']$/g,''); if(lam && eng &&!lam.startsWith('{') && lam.toLowerCase()!=='undefined' && eng.toLowerCase()!=='undefined'){ imported.push({lamnso:lam, english:eng, pos:'n', id:Date.now()+Math.random()}); } } } } imported=clean(imported); if(!imported.length){ document.getElementById('importStatus').innerText='No valid words.'; return; } let existing=new Set(localData.map(function(w){return w.lamnso.toLowerCase()})); let added=0; for(let k=0;k<imported.length;k++){ let w=imported[k]; if(!existing.has(w.lamnso.toLowerCase())){ localData.push(w); existing.add(w.lamnso.toLowerCase()); added++; } } saveLocal(); render(); document.getElementById('importStatus').innerText='Parsed '+imported.length+' | Added '+added+' new to LOCAL only.'; document.getElementById('status').innerText='LOCAL - Imported, not yet pushed'; }
...
function viewDuplicates(){ let seen={}; let dups=[]; localData.forEach(function(w){ let k=(w.lamnso||'').toLowerCase(); if(seen[k]) dups.push(w); else seen[k]=1 }); alert(dups.length? 'Duplicates ('+dups.length+'): '+dups.map(function(d){return d.lamnso}).join(', ') : 'No duplicates'); }
function mergeDuplicates(){ let map={}; localData.forEach(function(w){ let k=(w.lamnso||'').toLowerCase(); if(!map[k]) map[k]=w; else map[k].english+='; '+w.english }); localData=Object.values(map); saveLocal(); render(); alert('Merged! Now '+localData.length+' unique'); }
function removeDuplicates(){ let map={}; localData.forEach(function(w){ let k=(w.lamnso||'').toLowerCase(); map[k]=w }); let before=localData.length; localData=Object.values(map); saveLocal(); render(); alert('Removed '+(before-localData.length)+' duplicates!'); }
function sortAZ(){ localData.sort(function(a,b){return (a.lamnso||'').localeCompare(b.lamnso||'')}); saveLocal(); render(); }
function startLearn(){ if(!localData.length) return alert('No words'); let i=0; let show=function(){ let w=localData[i]; alert((i+1)+'/'+localData.length+'\\n'+w.lamnso+' = '+w.english); i=(i+1)%localData.length }; show(); let t=setInterval(function(){ if(confirm('Next word?')) show(); else clearInterval(t) },150); }
async function checkAccess(){ let code=document.getElementById('accessCode').value.trim(); if(!code) return alert('Enter code'); let r=await fetch('/api/check-access',{method:'POST', body:JSON.stringify({code:code}), headers:{'Content-Type':'application/json'}}); let j=await r.json(); document.getElementById('subStatus').innerText=j.ok?'Access Valid 365 days':'Invalid / Expired'; if(j.ok){ localStorage.setItem('premium_code_ok','1'); document.getElementById('paywall').style.display='none'; checkTrial(); } }
async function subscribe(){ let email=document.getElementById('subEmail').value; let phone=document.getElementById('subPhone').value; let code=document.getElementById('accessCode').value; if(!code) return alert('Enter Activation Code after payment to +237 674 061 571'); let r=await fetch('/api/subscribe',{method:'POST', body:JSON.stringify({email:email,phone:phone,code:code}), headers:{'Content-Type':'application/json'}}); let j=await r.json(); if(j.ok){ localStorage.setItem('premium_code_ok','1'); alert('Subscribed! Valid 365 days'); document.getElementById('paywall').style.display='none'; checkTrial(); } else { alert('Error '+(j.error||'Invalid code')); } }
async function generateCode(){ let adminKey = prompt('Enter Admin Key (ending 2026):'); if(!adminKey) return; let email = prompt('Customer email (optional):') || ''; let phone = prompt('Customer phone (optional):') || ''; let days = prompt('Days (default 365):') || '365'; let r=await fetch('/api/generate-code',{method:'POST', body:JSON.stringify({adminKey:adminKey, email:email, phone:phone, days: parseInt(days)}), headers:{'Content-Type':'application/json'}}); let j=await r.json(); if(j.ok){ document.getElementById('accessCode').value=j.code; document.getElementById('subStatus').innerText='Generated: '+j.code+' ('+j.days+' days)'; alert('Code Generated: '+j.code); } else { alert('Failed '+(j.error||'Failed')); } }
async function unlock(){ let code=document.getElementById('payCode').value.trim(); if(!code) return alert('Enter code'); let r=await fetch('/api/check-access',{method:'POST', body:JSON.stringify({code:code}), headers:{'Content-Type':'application/json'}}); let j=await r.json(); if(j.ok){ localStorage.setItem('premium_code_ok','1'); document.getElementById('paywall').style.display='none'; checkTrial(); alert('Unlocked!'); } else { alert('Invalid / Expired code. MoMo: 674 061 571'); } }
</script>
</body>
</html>`;
